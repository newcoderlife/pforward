...
func TestMultiForward(t *testing.T) {
	...
	f1, ok := handlers[0].(*PForward)
	if !ok {
		t.Fatalf("expected first plugin to be Forward, got %t", handlers[0])
	}
